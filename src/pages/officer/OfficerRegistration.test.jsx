...
function routes(overrides = {}) {
  return {
    ...
    "GET /reference/registration-crops": crops,
    "GET /reference/registration-centres": (call) =>
      call.search.includes(AGRA)
        ? registrationCentresAgra
        : registrationCentres,
    ...overrides,
  };
...
async function ready() {
  await waitFor(() => {
    expect(
      within(box("District")).getByRole("option", { name: "Aligarh" }),
    ).toBeInTheDocument();
  });
}
...
describe("/staff-register — District -> Centre, crops independent", () => {
  it("keeps the centre shut until a district is chosen, and lists the full crop catalog regardless", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("checkbox", { name: "Wheat" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("checkbox", { name: "Paddy" }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("fetches centres with the district UUID — not the district name", async () => {
    ...
    await waitFor(() => {
      expect(
        api.lastCall("GET", "/reference/registration-centres"),
      ).not.toBeNull();
    });
  ...
  it("keeps crop selections when the district or centre changes", async () => {
    ...
    await user.selectOptions(
      box("Procurement Centre"),
      registrationCentres[0].id,
    );
  ...
  it("requires at least one crop", async () => {
    ...
    expect(
      await screen.findByText(/select at least one crop/i),
    ).toBeInTheDocument();
  ...
  it("surfaces a centre-load failure and recovers on retry", async () => {
    ...
    await waitFor(() => {
      expect(options("Procurement Centre")).toContain(
        "Aligarh Demonstration Procurement Centre",
      );
    });
  });
  ...
  it("says so plainly when a district lists no active centre", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/No active centre is listed in this district/i),
      ).toBeInTheDocument();
    });
  });
});
